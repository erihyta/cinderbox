...
void Compress( std::array<uint32_t, 8>& h, const uint8_t* block )
{
	...
	for ( int i = 0; i < 64; ++i )
	{
		...
		uint32_t ch = ( e & f ) ^ ( ~e & g );
		uint32_t t1 = k + s1 + ch + kRound[size_t( i )] + w[i];
...
std::string Sha256Hex( const void* data, size_t size )
{
	...
	{
		Compress( h, bytes + 64 * i );
	}

	// The last partial block, the 0x80 marker and the length in bits (big-endian).
	uint8_t tail[128] = {};
	...
	for ( int i = 0; i < 8; ++i )
	{
		tail[tailSize - 1 - size_t( i )] = uint8_t( bits >> ( 8 * i ) );
	}
	Compress( h, tail );
	if ( tailSize == 128 )
	{
